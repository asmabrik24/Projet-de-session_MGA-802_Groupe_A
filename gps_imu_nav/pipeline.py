...
# Module principal du pipeline GPS/IMU.
# Il gère le chargement des fichiers bruts, le prétraitement,
# la synchronisation des données et la sauvegarde du dataset final.
...
# Classe orchestrant les différentes étapes du pipeline de fusion GPS/IMU.
class FusionPipeline:
    ...
    def __init__(self) -> None:
        # Construction des chemins principaux du projet et des fichiers source.
    ...
    # Exécute le pipeline complet depuis le chargement jusqu'à la sauvegarde finale.
    def run(self) -> None:
        print("\n====================================")
        ...
        # Le fichier GPS est indispensable pour démarrer le pipeline.
        ...
        # Le fichier IMU est optionnel : le pipeline peut continuer en mode GPS seul.
        imu = None
        ...
        # Nettoyage et normalisation des données GPS.
        gps = preprocess_gps(gps)
        print("[OK] GPS nettoyé et structuré")

        # Prétraitement IMU si le fichier a bien été chargé.
        if imu is not None:
            try:
        ...
        # Fusion temporelle des jeux de données GPS et IMU.
        data = synchronize_data(gps, imu)
        print("[OK] Synchronisation terminée")

        print("\n[4] Nettoyage final du dataset...")

        # Nettoyage final pour supprimer les doublons et remettre les données dans l'ordre temporel.
        ...
        # Sauvegarde du dataset final dans deux formats pour réutilisation ultérieure.
